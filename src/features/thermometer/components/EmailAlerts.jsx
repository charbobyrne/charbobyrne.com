    ...
    <section className="card thermometerEmailPanel" aria-labelledby="email-alert-title">
      ...
      {!token ? (
        <form onSubmit={connect} className="thermometerEmailForm">
          ...
          <small>Use your project control token. It stays in this pageâ€™s memory.</small>
          <button type="submit" disabled={busy}>{busy ? "Connectingâ€¦" : "Connect email alerts"}</button>
        </form>
      ) : (
        <form onSubmit={save} className="thermometerEmailForm">
          ...
          <div className="thermometerEmailLimits">
            ...
            <label>Maximum (Â°C)<input name="maxC" type="number" min="-55" max="125" step="any" value={draft.maxC} onChange={updateDraft} required /></label>
          </div>
          ...
          <div className="thermometerEmailActions">
            <button type="submit" disabled={busy}>{busy ? "Savingâ€¦" : "Save email settings"}</button>
